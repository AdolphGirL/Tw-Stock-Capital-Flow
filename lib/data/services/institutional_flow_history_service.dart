...
/// 三大法人每日流向歷史存取。
/// 資料存於 daily 目錄下的 institutional_flow_history.json（不受快照清理影響）。
class InstitutionalFlowHistoryService {
  static const _filename = 'institutional_flow_history.json';
  ...
  static Future<void> save(
    ...
    try {
      ...
      dev.log('[法人歷史] 已儲存 ${result.date}，共 ${kept.length} 筆', name: 'InstitutionalFlowHistory');
    } catch (e) {
      dev.log('[法人歷史] 儲存失敗（不影響主流程）: $e', name: 'InstitutionalFlowHistory');
    }
  }

  /// 讀取歷史，由舊到新排序（index 0 = 最舊）。
  static Future<List<InstitutionalFlowResult>> loadRecent(
    StorageService storage, {
    int limit = _maxKeep,
  }) async {
  ...
  static Future<List<InstitutionalFlowResult>> _load(StorageService storage) async {
    try {
      ...
      return rawList
          ...
          .map(InstitutionalFlowResult.fromJson)
          .toList();
    } catch (e) {
      dev.log('[法人歷史] 讀取失敗: $e', name: 'InstitutionalFlowHistory');
      return [];
    }
  }
}
